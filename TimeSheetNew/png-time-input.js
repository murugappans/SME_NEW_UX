  ...
  function TimeInputDirective() {
    ...
    function timepickerLinkFn(scope, element, attr) {
      ...
      function modelWatcher(dt) {
        ...
        console.log('2017-06-01T'+dt)

        if (angular.isString(dt)) {
            var temDate = '2017-06-01T'+dt;
            dt = new Date(temDate);
        }
      ...
      scope.$watch(function () {
        ...
        function inputWatcher(value) {
          ...
          //scope.model = new Date('2017-06-01T17:00:00');
          //scope.model.setHours(dateParts[0]);
          //scope.model.setMinutes(dateParts[1]);
          //scope.model.setSeconds(0);

      var temp = new Date();
      temp.setHours(dateParts[0]);
      temp.setMinutes(dateParts[1]);
      temp.setSeconds(0);
      ...
      if (temp.getHours() > 9) {
          ...
          hours = '0' + temp.getHours();
       
      }
        

      var minit = '0' + temp.getMinutes()
      if (temp.getMinutes() >9) {
          minit = temp.getMinutes();
      } else {
          minit = '0'+ temp.getMinutes();
      }
        


      var second = '0' + temp.getSeconds()
      if (temp.getSeconds() >9) {
          second = temp.getSeconds();
      } else {
          second = '0'+temp.getSeconds();
      }

        

      //var len = temp.getHours.length;
      //if (len < 2) {}
      //    hours= '0'+

      scope.model = hours + ':' + minit + ':' + second;
        });
